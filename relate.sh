...
#refpos=filtering/pos/victim.chr21.positionsOnly.txt
...
zcat $basedir/relate/allbams.mafs.gz |cut -f1,2 > $basedir/relate/posused.txt
zcat $basedir/relate/allbams.mafs.gz | cut -f5 |sed 1d > $basedir/relate/freq_from_pop

python scripts/getAF.py $basedir/relate/posused.txt 1000genomes_allele_freq.txt > $basedir/relate/freq.txt


n=$(wc -l $blist|cut -f1 -d" ")
/projects/korneliussen/apps/ngsRelate/ngsRelate -g $basedir/relate/allbams.glf.gz -n $n -f $basedir/relate/freq.txt -O $basedir/relate/ngsresults
